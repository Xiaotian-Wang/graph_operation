...
def search_graph_slow():
    """
    按照标准ID搜索图。
    输入： request.POST的form格式，KEY: 'keyword', VALUE: 字符串，其值为 标准的ID， 可以是多个，用英文的逗号','隔开， 逗号后不加空格。
    返回： 所有ID对应的标准的一跳图，以及所有节点间的直接关系。例如， 标准1的一跳节点与标准2的一跳节点之间若有关系，则该关系也会被返回。
...
def search_graph_fast():
    ...
    按照标准ID搜索图。
    输入： request.POST的form格式，KEY: 'keyword', VALUE: 字符串，其值为 标准的ID， 可以是多个，用英文的逗号','隔开， 逗号后不加空格。
    返回： 所有ID对应的标准的一跳图，以及标准节点间的直接关系。注意，标准1的一跳节点与标准2的一跳节点之间若有关系，则该关系可能不会被返回。
